...
public class RaffleStrategyController implements IRaffleStrategyService {
    ...
    public Response<Boolean> strategyArmory(@RequestParam Long strategyId) {
        try {
            ...
            Response<Boolean> response = Response.<Boolean>builder().code(ResponseCode.SUCCESS.getCode()).info(ResponseCode.SUCCESS.getInfo()).data(armoryStatus).build();
    ...
    public Response<List<RaffleAwardListResponseDTO>> queryRaffleAwardList(@RequestBody RaffleAwardListRequestDTO request) {
        try {
            log.info("查询抽奖奖品列表配开始 userId:{}activity:{}", request.getUserId(),request.getActivityId());
//            参数校验
...
//            获取规则配置
            String[] treeIds=strategyAwardEntities.stream().map(StrategyAwardEntity::getRuleModels).filter(ruleModel->ruleModel!=null&&!ruleModel.isEmpty()).toArray(String[]::new);
//          查询规则配置-获取奖品解锁限制，抽奖N次解锁
...
//            遍历填充数据
            ...
            for (StrategyAwardEntity strategyAward : strategyAwardEntities) {
                ...
                raffleAwardListResponseDTOS.add(RaffleAwardListResponseDTO.builder()
                        ...
                        .isAwardUnlock(awardRuleLockCount==null||awardRuleLockCount<=dayPartakeCount)
